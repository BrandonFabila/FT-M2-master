...
// Tu código acá:
const createdBy = document.querySelector("#createdBy");//crea el elemento al añadir el texto
createdBy.innerHTML += " Brandon Fabila";//se agrego el texto al elemento
...
ToDo.prototype.completeToDo = function () {
  this.complete = !this.complete;//si complete es true lo pasa a false y viceversa
}
...
function buildToDo(todo, index) {
  // Tu código acá:
  let toDoShell = document.createElement('div');//crea el div
  toDoShell.className = 'toDoShell';//asigna la clase

  let toDoText = document.createElement('span');//crea el elemento span
  toDoText.innerHTML = todo.description;//se asigna al todo description
  toDoText.id = index;//el id del to do es igual al index
...
function buildToDos(toDos) {
  // Tu código acá:
  let array = toDos.map(function(todo, i) {
    return buildToDo(todo, i);//
  });//devuelven un nuevo arreglo
  return array;
}
...
function displayToDos() {
  ...
  let build = buildToDos(toDoItems); //se pasa el argumento del array
  
  build.forEach(function(element) {
...
function addToDo() {
  ...
  if(input.value !== '') {
    let toDo = new ToDo(input.value);//la propiedad de descripcion sera igual input
    toDoItems.push(toDo);//actualiza el arreglo de toDos
    input.value = '';
    displayToDos();//hace que aparezca en pantalla
  }
}
...
add.addEventListener('click', addToDo);//se pasa la funcion al hacer click
...
function completeToDo(event) {
  ...
  // Tu código acá:
  toDoItems[index].completeToDo();//complete = falase  ==> true
  displayToDos();
